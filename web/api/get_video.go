...
func GetVideo() echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		if token != nil {
			...
			isFavoriteNotFound := dbs.DB.Table("favorites").Joins("INNER JOIN users ON user.id = favorites.user_id").Where(models.User{UID: token.UID}).Where(models.Favorite{VideoId: videoId}).First(&favorite).RecordNotFound()
